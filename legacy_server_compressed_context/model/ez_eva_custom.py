...
class JepaFeatureFusion(nn.Module):
    def __init__(self, embed_dim=1024, jepa_dim=1664, hidden_dim=4096, context_tokens=4, num_heads=8, num_layers=1, dropout=0.1):
        ...
        self.queries = nn.Parameter(torch.zeros(1, context_tokens, embed_dim))
        ...
        trunc_normal_(self.queries, std=.02)

    def forward(self, tracklet_feat, frame_feats, jepa_tokens):
        ...
        memory = self.adapter(jepa_tokens)
        queries = self.queries.expand(tracklet_feat.shape[0], -1, -1).to(dtype=tracklet_feat.dtype)
        context, _ = self.cross_attn(queries, memory, memory, need_weights=False)
        context = self.cross_norm(queries + context)
        tokens = torch.cat([tracklet_feat.unsqueeze(1), frame_feats, context], dim=1)
        return self.out_norm(self.fusion(tokens)[:, 0])
...
class EZ_Eva_Hybrid(EZ_Eva):
    ...
    def video_forward(self, x,cloth_id,jepa_tokens=None):
        x, B, T = self.forward_features(x,cloth_id)
        feat, feat_h = self.forward_head(x, B, T, pre_logits=True)
        feat_h = self.jepa_fusion(feat_h, feat, jepa_tokens) if self.jepa_fusion is not None else feat_h
        if not self.training:
            return feat_h
        else:
            cls_score = self.head(feat_h)
